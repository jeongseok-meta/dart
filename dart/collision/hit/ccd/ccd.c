...
static int __ccdGJK(const void *obj1, const void *obj2,
                    const dart_ccd_t *ccd, ccd_simplex_t *simplex);
...
static int doSimplex(ccd_simplex_t *simplex, dart_ccd_vec3_t *dir);
static int doSimplex2(ccd_simplex_t *simplex, dart_ccd_vec3_t *dir);
static int doSimplex3(ccd_simplex_t *simplex, dart_ccd_vec3_t *dir);
static int doSimplex4(ccd_simplex_t *simplex, dart_ccd_vec3_t *dir);

/** d = a x b x c */
inline void tripleCross(const dart_ccd_vec3_t *a, const dart_ccd_vec3_t *b,
                             const dart_ccd_vec3_t *c, dart_ccd_vec3_t *d);
...
static int nextSupport(const void *obj1, const void *obj2, const dart_ccd_t *ccd,
                       const ccd_pt_el_t *el,
                       ccd_support_t *out);



void dartCcdFirstDirDefault(const void *o1, const void *o2, dart_ccd_vec3_t *dir)
{
    dartCcdVec3Set(dir, CCD_ONE, CCD_ZERO, CCD_ZERO);
}

int dartCcdGJKIntersect(const void *obj1, const void *obj2, const dart_ccd_t *ccd)
{
    ccd_simplex_t simplex;
    return __ccdGJK(obj1, obj2, ccd, &simplex) == 0;
}

int dartCcdGJKSeparate(const void *obj1, const void *obj2, const dart_ccd_t *ccd,
                   dart_ccd_vec3_t *sep)
{
    ccd_pt_t polytope;
    ccd_pt_el_t *nearest;
    ...
    if (nearest)
        dartCcdVec3Copy(sep, &nearest->witness);
                     ...
                     dart_ccd_vec3_t *pos)
{
    ...
    dartCcdVec3Set(pos, CCD_ZERO, CCD_ZERO, CCD_ZERO);
    scale = CCD_ZERO;
    if (len % 2 == 1)
        len++;

    for (i = 0; i < len / 2; i++){
        dartCcdVec3Add(pos, &vs[i]->v.v1);
        dartCcdVec3Add(pos, &vs[i]->v.v2);
        scale += DART_CCD_REAL(2.);
    }
    dartCcdVec3Scale(pos, CCD_ONE / scale);
...
int dartCcdGJKPenetration(const void *obj1, const void *obj2, const dart_ccd_t *ccd,
                      dart_ccd_real_t *depth, dart_ccd_vec3_t *dir, dart_ccd_vec3_t *pos)
{
    ...
    if (ret == 0 && nearest){
        ...
        *depth = DART_CCD_SQRT(nearest->dist);

        // store normalized direction vector
        dartCcdVec3Copy(dir, &nearest->witness);
                    ...
                    const dart_ccd_t *ccd, ccd_simplex_t *simplex)
{
    ...
    dart_ccd_vec3_t dir; // direction vector
    ...
    for (iterations = 0UL; iterations < ccd->max_iterations; ++iterations) {
        ...
        if (dartCcdVec3Dot(&last.v, &dir) < CCD_ZERO){
...
static int doSimplex2(ccd_simplex_t *simplex, dart_ccd_vec3_t *dir)
{
    ...
    dartCcdVec3Sub2(&AB, &B->v, &A->v);
    // compute AO vector
    dartCcdVec3Copy(&AO, &A->v);
    dartCcdVec3Scale(&AO, -CCD_ONE);

    // dot product AB . AO
    dot = dartCcdVec3Dot(&AB, &AO);

    // check if origin doesn't lie on AB segment
    dartCcdVec3Cross(&tmp, &AB, &AO);
    if (ccdIsZero(dartCcdVec3Len2(&tmp)) && dot > CCD_ZERO){
        return 1;
    }
...
static int doSimplex3(ccd_simplex_t *simplex, dart_ccd_vec3_t *dir)
{
    ...
    dart_ccd_vec3_t AO, AB, AC, ABC, tmp;
    dart_ccd_real_t dot, dist;
    ...
    dist = dartCcdVec3PointTriDist2(ccd_vec3_origin, &A->v, &B->v, &C->v, NULL);
    if (ccdIsZero(dist)){
        return 1;
    }
    ...
    if (dartCcdVec3Eq(&A->v, &B->v) || dartCcdVec3Eq(&A->v, &C->v)){
        return -1;
    }
    ...
    dartCcdVec3Sub2(&AB, &B->v, &A->v);
    dartCcdVec3Sub2(&AC, &C->v, &A->v);
    dartCcdVec3Cross(&ABC, &AB, &AC);

    dartCcdVec3Cross(&tmp, &ABC, &AC);
    dot = dartCcdVec3Dot(&tmp, &AO);
    if (ccdIsZero(dot) || dot > CCD_ZERO){
        dot = dartCcdVec3Dot(&AC, &AO);
        if (ccdIsZero(dot) || dot > CCD_ZERO){
...
ccd_do_simplex3_45:
        ...
        dartCcdVec3Cross(&tmp, &AB, &ABC);
        dot = dartCcdVec3Dot(&tmp, &AO);
        if (ccdIsZero(dot) || dot > CCD_ZERO){
            goto ccd_do_simplex3_45;
        }else{
            dot = dartCcdVec3Dot(&ABC, &AO);
            if (ccdIsZero(dot) || dot > CCD_ZERO){
                dartCcdVec3Copy(dir, &ABC);
            }else{
                ccd_support_t Ctmp;
...
static int doSimplex4(ccd_simplex_t *simplex, dart_ccd_vec3_t *dir)
{
    ...
    dart_ccd_vec3_t AO, AB, AC, AD, ABC, ACD, ADB;
    int B_on_ACD, C_on_ADB, D_on_ABC;
    int AB_O, AC_O, AD_O;
    dart_ccd_real_t dist;
    ...
    // found
    dist = dartCcdVec3PointTriDist2(&A->v, &B->v, &C->v, &D->v, NULL);
    if (ccdIsZero(dist)){
        return -1;
    }
    ...
    if (ccdIsZero(dist))
        return 1;
    dist = dartCcdVec3PointTriDist2(ccd_vec3_origin, &A->v, &C->v, &D->v, NULL);
    if (ccdIsZero(dist))
        return 1;
    dist = dartCcdVec3PointTriDist2(ccd_vec3_origin, &A->v, &B->v, &D->v, NULL);
    if (ccdIsZero(dist))
        return 1;
    dist = dartCcdVec3PointTriDist2(ccd_vec3_origin, &B->v, &C->v, &D->v, NULL);
    if (ccdIsZero(dist))
        return 1;
    ...
    dartCcdVec3Sub2(&AB, &B->v, &A->v);
    dartCcdVec3Sub2(&AC, &C->v, &A->v);
    dartCcdVec3Sub2(&AD, &D->v, &A->v);
    dartCcdVec3Cross(&ABC, &AB, &AC);
    dartCcdVec3Cross(&ACD, &AC, &AD);
    dartCcdVec3Cross(&ADB, &AD, &AB);

    // side (positive or negative) of B, C, D relative to planes ACD, ADB
    // and ABC respectively
    B_on_ACD = ccdSign(dartCcdVec3Dot(&ACD, &AB));
    C_on_ADB = ccdSign(dartCcdVec3Dot(&ADB, &AC));
    D_on_ABC = ccdSign(dartCcdVec3Dot(&ABC, &AD));

    // whether origin is on same side of ACD, ADB, ABC as B, C, D
    // respectively
    AB_O = ccdSign(dartCcdVec3Dot(&ACD, &AO)) == B_on_ACD;
    AC_O = ccdSign(dartCcdVec3Dot(&ADB, &AO)) == C_on_ADB;
    AD_O = ccdSign(dartCcdVec3Dot(&ABC, &AO)) == D_on_ABC;
...
{
    dart_ccd_vec3_t e;
    dartCcdVec3Cross(&e, a, b);
    dartCcdVec3Cross(d, &e, c);
}
                              ...
                              ccd_pt_t *pt, ccd_pt_el_t **nearest)
{
    const ccd_support_t *a, *b, *c;
    ccd_support_t d, d2;
    dart_ccd_vec3_t ab, ac, dir;
    ccd_pt_vertex_t *v[5];
    ccd_pt_edge_t *e[9];
    dart_ccd_real_t dist, dist2;
    ...
    *nearest = NULL;
    ...
    dartCcdVec3Sub2(&ab, &b->v, &a->v);
    dartCcdVec3Sub2(&ac, &c->v, &a->v);
    dartCcdVec3Cross(&dir, &ab, &ac);
    __ccdSupport(obj1, obj2, &dir, ccd, &d);
    dist = dartCcdVec3PointTriDist2(&d.v, &a->v, &b->v, &c->v, NULL);

    // and second one take in opposite direction
    dartCcdVec3Scale(&dir, -CCD_ONE);
    __ccdSupport(obj1, obj2, &dir, ccd, &d2);
    dist2 = dartCcdVec3PointTriDist2(&d2.v, &a->v, &b->v, &c->v, NULL);
                              ...
                              ccd_pt_t *pt, ccd_pt_el_t **nearest)
{
    ...
    for (i = 0; i < ccd_points_on_sphere_len; i++){
        ...
        if (!dartCcdVec3Eq(&a->v, &supp[0].v) && !dartCcdVec3Eq(&b->v, &supp[0].v)){
            found = 1;
            break;
        }
    }
    ...
    dartCcdVec3Copy(&dir, &supp[0].v);
    dartCcdVec3Scale(&dir, -CCD_ONE);
    __ccdSupport(obj1, obj2, &dir, ccd, &supp[1]);
    if (dartCcdVec3Eq(&a->v, &supp[1].v) || dartCcdVec3Eq(&b->v, &supp[1].v))
        goto simplexToPolytope2_touching_contact;

    // next will be in direction of normal of triangle a,supp[0],supp[1]
    dartCcdVec3Sub2(&ab, &supp[0].v, &a->v);
    dartCcdVec3Sub2(&ac, &supp[1].v, &a->v);
    dartCcdVec3Cross(&dir, &ab, &ac);
    __ccdSupport(obj1, obj2, &dir, ccd, &supp[2]);
    if (dartCcdVec3Eq(&a->v, &supp[2].v) || dartCcdVec3Eq(&b->v, &supp[2].v))
        goto simplexToPolytope2_touching_contact;

    // and last one will be in opposite direction
    dartCcdVec3Scale(&dir, -CCD_ONE);
    __ccdSupport(obj1, obj2, &dir, ccd, &supp[3]);
    if (dartCcdVec3Eq(&a->v, &supp[3].v) || dartCcdVec3Eq(&b->v, &supp[3].v))
        goto simplexToPolytope2_touching_contact;
                       ...
                       ccd_support_t *out)
{
    ...
    // origin a bit.
    dist = dartCcdVec3Dot(&out->v, &el->witness);
